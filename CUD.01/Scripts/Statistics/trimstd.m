function m = trimmean(x,stddev,dim)
...
% Find the upper and lower stddeviles of X, and trim out values that are
% more extreme.
%zlo = repmat(prctile(x, (stddev / 2),dim), tile);
%zhi = repmat(prctile(x, 100 - stddev / 2,dim), tile);    
zlo = repmat(nanmean(x)-stddev*nanstd(x), tile);
zhi = repmat(nanmean(x)+stddev*nanstd(x), tile); 
x(x < zlo | zhi < x) = NaN
