...
username        = os.getlogin() # "yaxley" # Cluster username
experiment      = "CUD.01" # Experiment name for qsub
template_f      = file("PS.submit.sh") # Job template location (on head node)
fsf_template    = "PS.fsf"
output_dir      = "PS/4disdaqs"
runs = range(1,7) # range cuts the last number off
...
for s,g in sorted(subjects.iteritems(), reverse=True):
